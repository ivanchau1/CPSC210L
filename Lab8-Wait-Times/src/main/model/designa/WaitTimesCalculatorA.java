...
public class WaitTimesCalculatorA {
    ...
    // MODIFIES: this
    // EFFECTS: Adds the wait time to the provided date's group based on the
    // calculator's grouping setting
    ...
    // EFFECTS: returns the appropriate group name for the provided date based on
    // the grouping setting provided
    // for day of week: returns the day of the week (e.g. "Monday") that corresponds
    // with the record's time
    // otherwise: returns the hour of the day (e.g. "9AM") corresponding to the
    // record's time
    public String getGroup(WaitTimeRecord record) {
        ...
        switch (this.group) {
            ...
            case HOUR_OF_DAY:
                ...
                return getHourlyGroupNames().get(hourOfDay);
    ...
    // EFFECTS: returns the set of groups that, if applicable, must always be shown
    // even if values
    ...
    // EFFECTS: returns the display name / label of the grouping type. e.g. 'Average
    // wait times by ...'
    ...
    // EFFECTS: returns every hour of the day, in order from 12AM to 11PM clockwise
    // as a List of Strings
    private List<String> getHourlyGroupNames() {
        return Arrays.asList(
