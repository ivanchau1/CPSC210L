 ...
 * In order to stop the puppies from running away, the owner has to 'call' them
 * by typing their name.
 * <br>
 * <br>
 * The game runs under the following sequence: <br>
 * 1. The player starts calling a puppy by typing the first letter of its name.
 * This 'selects' the puppy. <br>
 * 2. The player must type the rest of its name to call it. Any incorrect input
 * (e.g. typing 'a' when the next letter is
 * 'c' is ignored) <br>
 * 3. Once the player has typed the last character, the puppy is now 'called'
 * and will stop running away. This also
 ...
 * - Typing the name for the current puppy - corresponding to step 2 and 3
 * above. <br>
 * - Finding a puppy whose next character starts with an input character c -
 * helpful for step 1. <br>
 * <br>
 * <br>
 * These pieces are finally put together in the handleInput method, where you
 * can use your work earlier
 * to handle the full logic of the game! As you write this method, think about
 * where and how the other
 * helper methods might be useful.
 */
public class PuppyCaller {

    private Puppy currentPuppySelected;

    // EFFECTS: creates an instance of this class with no current puppy selected.
    public PuppyCaller() {
        currentPuppySelected = null;
    }

    // EFFECTS: returns the current selected puppy, null if none is selected
    public Puppy getCurrentPuppy() {
        return currentPuppySelected;
    }
    ...
    public void setCurrentPuppy(Puppy currentPuppy) {
        currentPuppySelected = currentPuppy;
    }

    // REQUIRES: getCurrentPuppy() != null and !getCurrentPuppy().hasBeenCalled()
    // MODIFIES: this
    // EFFECTS: Types one character of the current puppy's name if c is the next
    // character to be typed for that puppy
    // If the puppy has been called after typing that character, current puppy will
    // be null
    // Returns whether progress was made on calling the puppy
    public boolean typeNameForCurrentPuppy(char c) {
        if (currentPuppySelected.getNextChar() == c) {
            currentPuppySelected.typeName();
            if (currentPuppySelected.hasBeenCalled()) {
                currentPuppySelected = null;
            }
            return true;
        }
    ...
    // REQUIRES: currentPuppies.size() > 0 AND for all puppies in the list, puppy
    // has not been called
    ...
    public boolean handleInput(List<Puppy> currentPuppies, char c) {
        if (currentPuppySelected != null) {
            return typeNameForCurrentPuppy(c);
        } else {
            if (findPuppy(currentPuppies, c) == null) {
                return false;
            } else {
                currentPuppySelected = findPuppy(currentPuppies, c);
                return typeNameForCurrentPuppy(c);
            }
        }
    }
